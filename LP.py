...
class LP:
    ...
    def AssertValidFormatting(self):
        ...
        assert (self.A_eq is not None) == (
            self.b_eq is not None
        ), "If you provide A_eq or b_eq you must provide both"
        assert (self.A_leq is not None) == (
            self.b_leq is not None
        ), "If you provide A_leq or b_leq you must provide both"
        ...
        if self.A_eq is not None:
            ...
            assert numEqConstr == len(
                self.b_eq
            ), "A_eq and b_eq must indicate the same number of constraints"
        ...
        if self.A_leq is not None:
            ...
            assert nVar == numVar, "c and A_leq have conflicting numbers of variables"
            assert numLeqConstr == len(
                self.b_leq
            ), "A_leq and b_leq must indicate the same number of constraints"

        if self.lb is not None:
            assert len(self.lb.shape) == 1, "lb must be a one-dimensional array"
            assert numVar == len(
                self.lb
            ), "c and lb have conflicting numbers of variables"

        if self.ub is not None:
            assert len(self.ub.shape) == 1, "ub must be a one-dimensional array"
            assert numVar == len(
                self.ub
            ), "c and ub have conflicting numbers of variables"
    ...
    def __init__(
        ...
        if A_eq is not None:
            ...
            self.A_eq = np.zeros((0, numVar), dtype=float)
        ...
        if A_leq is not None:
            ...
            self.A_leq = np.zeros((0, numVar), dtype=float)
