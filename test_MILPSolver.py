...
def test_small3d():
    A = np.array([[0.1, -1, 0], [0.1, 1, -1], [1, 0, 10], [0.3, -0.8, 2], [0, 0, 1]])
...
def GenerateRandomMILP(numVar, numConstr, lb=-10, ub=10, fracInt=0.5):
    ...
    xFeas[integerVars] = np.round(xFeas[integerVars])

    b = A @ xFeas

    milp = MILP(c, integerVars, A, b, lb=np.ones(numVar) * lb, ub=np.ones(numVar) * ub)
    return milp
...
def assertCorrect(milp):
    ...
    if scipyAnswer is None:
        assert myAnswer.terminationCondition == TerminationCondition.INFEASIBLE
        return
